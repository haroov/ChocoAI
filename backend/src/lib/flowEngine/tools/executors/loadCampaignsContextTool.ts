 ...
 */
export const loadCampaignsContextTool: ToolExecutor = async (payload, { conversationId }) => {
  try {
    ...
    const forceLive =
      (payload && typeof (payload as any).force_live !== 'undefined' && Boolean((payload as any).force_live)) ||
      (payload && typeof (payload as any).forceLive !== 'undefined' && Boolean((payload as any).forceLive));
    ...
    let campaignsSource: 'cache' | 'live' = 'live';
    if (!forceLive && cachedRow?.value) {
      try {
        const parsed = JSON.parse(cachedRow.value);
        campaigns = Array.isArray(parsed) ? parsed : [];
        if (campaigns.length) campaignsSource = 'cache';
      } catch {
        // fall back to live fetch
      }
    }

    if (forceLive || !campaigns.length) {
      ...
      const json = await trackApiCall(
        ...
        'get-campaigns',
        {
          payload: { orgId, forceLive },
          meta: { method: 'GET', endpoint },
        },
        async () => {
          ...
          if (!res.ok) {
            ...
            if (res.status === 401 || res.status === 403) {
              return { errorCode: 'JWT_INVALID', error: 'Authorization failed', status: res.status } as any;
            }
            return { errorCode: 'CAMPAIGNS_FETCH_FAILED', error: 'Failed to load campaigns', status: res.status } as any;
          }

          return await res.json();
        },
      ).catch((err) => (
        // trackApiCall throws on error; normalize to tool result below
        { error: err?.message || String(err) } as any
      ));

      if ((json as any)?.errorCode === 'JWT_INVALID') {
        return { success: false, errorCode: 'JWT_INVALID', error: 'Authorization failed' };
      }
      if ((json as any)?.errorCode === 'CAMPAIGNS_FETCH_FAILED') {
        return { success: false, errorCode: 'CAMPAIGNS_FETCH_FAILED', error: 'Failed to load campaigns' };
      }
      if ((json as any)?.error && !(json as any)?.data) {
        return { success: false, errorCode: 'CAMPAIGNS_FETCH_FAILED', error: (json as any).error };
      }

      campaigns = Array.isArray((json as any)?.data) ? (json as any).data : (Array.isArray(json) ? (json as any) : []);
      campaignsSource = 'live';
    }
    ...
    const campaignsCount = normalized.length;

    // Upcoming/scheduled campaigns (start_date in the future)
    const now = Math.floor(Date.now() / 1000);
    const scheduled = normalized
      .filter((c) => typeof c.start_date === 'number' && Number.isFinite(c.start_date) && c.start_date > now)
      .sort((a, b) => Number(a.start_date) - Number(b.start_date));
    const upcoming = scheduled[0];
    ...
    const saveResults = {
      ...
      has_upcoming_campaign: hasUpcoming,
      upcoming_campaign_id: hasUpcoming ? String(upcoming.id) : '',
      upcoming_campaign_title: hasUpcoming ? String(upcoming.title || '').trim() : '',
      upcoming_campaign_start_date: hasUpcoming ? Number(upcoming.start_date) : 0,
