  ...
  definition: {
    stages: {
      collectBasics: {
        ...
        action: {
          // Deterministic intent gate: default to "quote" when user asks for a quote,
          // and only ask account/policy confirmation when truly ambiguous.
        ...
        prompt: [
          ...
          '',
          'Speak like a human (avoid formal "אנא ספק לי").',
          'Do NOT use phrasing like "תוכל/י לספק לי...". Ask as direct human questions.',
          '',
          'CRITICAL: For the first message in this stage, use this exact Hebrew copy (or extremely close):',
          '"הי, אני שוקו — סוכן הביטוח הדיגיטלי שלך. אוכל לתת לך הצעה לביטוח לעסק או לעזור בשירות על פוליסה קיימת. מה השם הפרטי והמשפחה שלך ומה מספר הנייד? ובקצרה — איך אוכל לעזור?"',
          '',
          '- English example:',
          '"Hi, I\'m Choco, your digital insurance agent. I can help with a new business insurance quote or service for an existing policy. What\'s your first name, last name, and mobile number — and how can I help?"',
          '',
        ...
        nextStage: {
          conditional: [
            { condition: 'needs_account_confirmation === true', ifTrue: 'intent' },
          ],
          fallback: 'emailAndSegment',
        },
      },
      emailAndSegment: {
        name: 'Email + segment',
        description: 'Collect email to proceed. Segment is captured implicitly when provided early.',
        prompt: [
          'LANGUAGE: Maintain same language throughout. Hebrew → Hebrew. English → English.',
          '',
          ...
          '',
          'Ask ONLY for the email (if missing).',
          'CRITICAL:',
          '- Do NOT re-ask whether this is a new quote if the user already asked for a quote (default is new quote).',
          '- Do NOT justify ("כדי ש...", "כדי לשלוח הצעה").',
          '',
          'Use this exact copy:',
          '"תודה, {first_name}! מה כתובת האימייל שלך?"',
        ].join('\n'),
        fieldsToCollect: ['email'],
        nextStage: 'complete',
      },
      intent: {
        name: 'Intent',
      ...
      complete: {
        ...
        action: {
          ...
          // Always route once we reach this stage.
          // The route tool itself uses a safe default: if intent_type is missing/unknown, it routes to needsDiscovery (new quote).
          condition: 'true',
        },
        prompt: [
    ...
    fields: {
      ...
      segment_description: {
        type: 'string',
        description: 'Plain-language segment description (e.g., flower shop, yoga teacher).',
      },
      intent_confidence: {
        type: 'number',
